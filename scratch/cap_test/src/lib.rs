...
mod invariant_tests {
    ...
    /// ALL_APPS_INV-1: 6 essential desktop icons registered with correct actions
    #[test]
    fn test_all_apps_inv_1_six_desktop_icons_registered() {
        ...
        let icons = [
            ...
            (5, "Settings", Action::OpenSettings),
            (6, "Trash", Action::OpenTrash),
        ];
    ...
    fn test_all_apps_inv_2_double_click_activation() {
        let last_click_id: Option<u32> = Some(3); // Editor
    ...
    /// ALL_APPS_INV-3: Application Launcher contains 6 streamlined registered apps
    #[test]
    fn test_all_apps_inv_3_launcher_contains_all_registered_apps() {
        let apps = [
            (1, "Terminal", "terminal.app"),
            (2, "Files", "files.app"),
            (3, "Text Editor", "editor.app"),
            (4, "Task Manager", "taskmgr.app"),
            (5, "Settings", "settings.app"),
            (6, "Web Browser", "browser.app"),
        ];

        assert_eq!(apps.len(), 6);
        assert_eq!(apps[2].1, "Text Editor");
        assert_eq!(apps[3].1, "Task Manager");
    }

    /// ALL_APPS_INV-4: 1:1 Aspect-Preserved Fast Row Copy Blitting (Zero Lag)
    #[test]
    fn test_all_apps_inv_4_fast_one_to_one_row_copy_blitting() {
        let surf_w = 420usize;
        let surf_h = 260usize;
        let target_w = 1280usize; // Maximized or large window
        let target_h = 696usize;

        let copy_w = surf_w.min(target_w);
        let copy_h = surf_h.min(target_h);

        assert_eq!(copy_w, 420);
        assert_eq!(copy_h, 260);

        // Verify that only 260 fast row copies occur instead of 1280 * 696 (890,880) pixel loops
        let num_row_copies = copy_h;
        assert_eq!(num_row_copies, 260);
    }

    /// ALL_APPS_INV-5: Fullscreen Centered Letterbox Geometry
    #[test]
    fn test_all_apps_inv_5_fullscreen_centered_geometry() {
        let screen_w = 1280usize;
        let screen_h = 720usize;
        let surf_w = 420usize;
        let surf_h = 260usize;

        let offset_x = screen_w.saturating_sub(surf_w) / 2;
        let offset_y = screen_h.saturating_sub(surf_h) / 2;

        assert_eq!(offset_x, (1280 - 420) / 2); // 430
        assert_eq!(offset_y, (720 - 260) / 2);  // 230

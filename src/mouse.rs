...
pub async fn mouse_task() {
    ...
    loop {
        ...
        if click && !last_click {
            ...
            // Desktop Icon Tıklama Kontrolu (Terminal: x=20, y=20, w=40, h=40)
            if c_x >= 20 && c_x <= 60 && c_y >= 20 && c_y <= 60 {
                if !w.visible {
                    w.visible = true;
            ...
            // Buyut / Kucult Butonu (O)
                ...
                if w.win_w < 1280 {
                    ...
                    w.win_y = 0;
                    w.win_w = 1280;
                    w.win_h = 690; // Taskbar payi (720-30)
                    crate::gui::draw_desktop_and_window(w.win_x, w.win_y, w.win_w, w.win_h, w.visible);
                } else {
                    // Restore
                    w.win_x = 100;
            ...
            if drag.mode != 0 {
                ...
                // Icerigi off-screen buffera (1280x720 otesine) kopyala
                unsafe {
                    let offscreen = crate::gui::VESA.framebuffer.add(1280 * 720);
                    for i in 0..(w.win_h - 32) {
                        for j in 0..(w.win_w - 8) {
                            let src_offset = ((w.win_y + 28 + i) as usize) * 1280 + ((w.win_x + 4 + j) as usize);
                            let dst_offset = (i as usize) * 1280 + (j as usize);
                ...
                if drag.mode == 1 {
                    ...
                    w.win_w = (drag.win_start_w as i32 + diff).max(300).min(1280) as u16;
                } else if drag.mode == 3 {
                    let diff = cy as i32 - drag.start_y as i32;
                    w.win_h = (drag.win_start_h as i32 + diff).max(200).min(690) as u16;
                } else if drag.mode == 4 {
                    let diff_x = cx as i32 - drag.start_x as i32;
                    let diff_y = cy as i32 - drag.start_y as i32;
                    w.win_w = (drag.win_start_w as i32 + diff_x).max(300).min(1280) as u16;
                    w.win_h = (drag.win_start_h as i32 + diff_y).max(200).min(690) as u16;
                }
                ...
                unsafe {
                    ...
                    for i in 0..copy_h {
                        for j in 0..copy_w {
                            let src_offset = (i as usize) * 1280 + (j as usize);
                            let dst_offset = ((w.win_y + 28 + i) as usize) * 1280 + ((w.win_x + 4 + j) as usize);
